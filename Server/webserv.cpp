...
void WebServ::HandleEstablishedConnections(SocketConnection *Connection, int16_t  filter)
{
    ...
    if (filter == EVFILT_READ)
    {
        ...
        if (Connection->request.method.empty())
        {
        ...
        if (Connection->content_size <= 0)
        {
            // AddEvent(Connection->socket_fd, EVFILT_READ, EV_DELETE);
            // AddEvent(Connection->socket_fd, EVFILT_WRITE, EV_ENABLE);
